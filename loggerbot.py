...
TOKEN = '7292835940:AAFTXBNhLlFt_CFuQchN1QBO-2XdaPanC3k'
...
def send_welcome(message):
    ...
    markup.row(
        InlineKeyboardButton('ℹ️ Канал', callback_data='authors'),
...
@bot.callback_query_handler(func=lambda call: call.data == 'hueta')
def show_hueta(call):
    bot.send_message(call.message.chat.id, "hueta")

@bot.callback_query_handler(func=lambda call: call.data == 'authors')
def show_authors(call):
    bot.send_message(call.message.chat.id, "Канал создателя: @tupoydayn")

@bot.callback_query_handler(func=lambda call: call.data == 'templates')
def show_templates(call):
    markup = InlineKeyboardMarkup()
    markup.row(
        InlineKeyboardButton('👀 Деанон', callback_data='template_deanon'),
    )
    markup.row(
        InlineKeyboardButton('🚨 Сват', callback_data='template_swat')
    )
